...
export const useAuthStore = defineStore('auth', {
  ...
  actions: {
    ...
    async login(username: string, password: string, remember: boolean, totpCode = '', captchaID = '', captcha = '') {
      const result = await api<{ user: User }>('/api/auth/login', { method: 'POST', body: json({ username, password, remember, totpCode, captchaID, captcha }) })
